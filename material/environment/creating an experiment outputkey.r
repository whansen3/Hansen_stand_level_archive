...
master=read.csv("master_environment_key.csv",header=T,sep=",") # the master table
summary(master)
str(master)
...
for(i in 1:dim(m.1)[1])
	...
	env=rbind(env,data.frame(
		...
		model.soil=m.1$soil[i],
		model.stand.age=m.1$stand_age[i]
		))
	}

summary(env)
str(env)
write.table(env,file="experiment_key.txt",col.names=T,row.names=F,quote=F)